...
JQ_CLI=$(which jq) || {
  echo "ERROR: jq not installed.  Please install jq the CLI JSON Processor"
  return 1
}
...
function mfa() {
  ...
  [ $# -eq 0 ] && {
    echo "Usage: $0 token-code [profile]"
    return 1
  }
  [ $# -gt 3 ] && {
    echo "Usage: $0 token-code [profile]"
  ...
  ARN_OF_MFA=$(${AWS_CLI} configure get aws_mfa_device --profile "${AWS_PROFILE}") ||
    {
      "Error: AWS CLI failed to retrieve the MFA ARN for profile ${AWS_PROFILE}"
      return 1
    }
  ...
  CREDS=$(${AWS_CLI} sts get-session-token --output json --duration-seconds "${AWS_CREDENTIAL_DURATION_SECONDS}" --serial-number "${ARN_OF_MFA}" --profile "${AWS_PROFILE}" --token-code "${TOKEN_CODE}") ||
    {
      "Error: AWS CLI failed to get a credential"
      return 1
    }
...
function mfa_cache() {
  ...
  [ $# -eq 1 ] && {
    AWS_CREDS_CACHE="${HOME}/.aws/.credcache.${1}"
    AWS_PROFILE="${1}"
  }
  [ $# -gt 1 ] && {
...
function mfa_cache_list() {
  ...
  pushd . >/dev/null
  cd "${HOME}/.aws/" || {
    echo "ERROR: Unable to cd to ${HOME}/.aws/"
    return 1
  }
  for filename in .credcache.*; do
    echo $filename | ${SED_CLI} 's/\.credcache\.//g'
  done
  popd >/dev/null || {
    echo "ERROR: Unable to return to your current working directory"
    return 1
  }
  unset SED_CLI
}
